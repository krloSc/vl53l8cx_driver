...
LOG_MODULE_REGISTER(st_vl53l8cx_api, CONFIG_SENSOR_LOG_LEVEL);
              ...
              VL53L8CX_Configuration      *p_dev)
{
   ...
   do {
	LOG_DBG("Trying to read MCU boot status...\n");
		...
		VL53L8CX_Configuration		*p_dev)
{
	...
	LOG_DBG("Enabling FW access and downloading FW...\n");
	/* Enable FW access */
	...
	LOG_DBG("Enabling host access to GO1 and downloading FW...\n");
	/* Enable host access to GO1 */
	status |= VL53L8CX_RdByte(&(p_dev->platform), 0x7fff, &tmp);
	status |= VL53L8CX_WrByte(&(p_dev->platform), 0x0C, 0x01);

	LOG_DBG("Powering ON the sensor and downloading FW...\n");
	/* Power ON status */
	status |= VL53L8CX_WrByte(&(p_dev->platform), 0x7fff, 0x00);
	...
	LOG_DBG("Waking up the MCU and downloading FW...\n");
	/* Wake up MCU */
	status |= VL53L8CX_WrByte(&(p_dev->platform), 0x7fff, 0x00);
	status |= VL53L8CX_RdByte(&(p_dev->platform), 0x7fff, &tmp);
	status |= VL53L8CX_WrByte(&(p_dev->platform), 0x7fff, 0x01);

	LOG_DBG("Downloading FW into VL53L8CX...\n");
	...
	LOG_DBG("Checking FW checksum and downloading FW...\n");
	...
	LOG_DBG("Resetting MCU and waiting for boot...\n");
	...
	LOG_DBG("Waiting for MCU boot...\n");
	status |= _vl53l8cx_poll_for_mcu_boot(p_dev);
	if(status != (uint8_t)0){
		goto exit;
	}
	(void)VL53L8CX_WaitMs(&(p_dev->platform), 10);

	status |= VL53L8CX_WrByte(&(p_dev->platform), 0x7fff, 0x02);

	LOG_DBG("VL53L8CX firmware booted correctly, checking checksum\n");
	/* Firmware checksum */
	...
	LOG_DBG("preparing offset and NVM\n");
	...
	LOG_DBG("Sending default Xtalk to VL53L8CX...\n");
	...
	LOG_DBG("Sending default configuration to VL53L8CX...\n");
